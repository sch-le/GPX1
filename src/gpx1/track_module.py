...
def _get_trkpts(input_gpx: gpxpy.gpx.GPX) -> list:
    """Creates a list of all trackpoints and their associated latitude, longitude, and optional elevation.
...
def print_list(input_gpx: gpxpy.gpx.GPX) -> None:
    """Prints a list of all trackpoints and their associated latitude, longitude, and optional elevation.
    ...
    for id, trkpt in enumerate(trkpts):
        if trkpt[2] != "":
            print(f"  {id:04}  |  {trkpt[0]:4.6f} |  {trkpt[1]:4.6f}  | {trkpt[2]:4.6f}")
        else:
            print(f"  {id:04}  |  {trkpt[0]:4.6f} |  {trkpt[1]:4.6f}  | {trkpt[2]}")
...
def edit(id: int, lat: float, lon: float, ele: float, input_gpx: gpxpy.gpx.GPX) -> gpxpy.gpx.GPX:
    ...
    Args:
        id (int): ID of the trackpoint to be edited
        lat (float): Latitude
        lon (float): Longitude
        ele (float): Elevation
        input_gpx (gpxpy.gpx.GPX): Data from the GPX file
